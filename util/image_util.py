...
class ImageURL:
    ...
    def download_image(self, url=None, path=None, thumbnail=True, keep_original=False):
        if not url:
            url = self.url
        if not path:
            path = self.path
    ...
    def resize(self, width, height, keep_aspect_ratio=True, src_path=None, dst_path=None): # Only works for shrinking size
        ...
        if size != tuple([width, height]):
            return self._padding(width, height, src_path=src_path, dst_path=dst_path)
        return img.size
    ...
    def _padding(self, width, height, src_path=None, dst_path=None, fill=DEFAULT_FILLING):
        ...
        new_img = Image.new("RGB", (width, height), fill)
    ...
    def expand(self, width, height, keep_aspect_ratio=True, src_path=None, dst_path=None, fill=DEFAULT_FILLING):
        ...
        new_img = ImageOps.expand(img,
                                  (width_delta//2, height_delta//2, width_delta-(width_delta//2), height_delta-(height_delta//2)),
                                  fill)
        self._create_parent_folders(dst_path)
        new_img.save(dst_path)
        if size != img.size:
